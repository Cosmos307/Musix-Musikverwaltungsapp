...
public class Books
{
    ...
    [OpenApiOperation(operationId: "listBooks", tags: new[] {"books"}, Summary = "List Books", Description = "Get list of books.")]
    [OpenApiSecurity("function_key", SecuritySchemeType.ApiKey, Name = "code", In = OpenApiSecurityLocationType.Query)]
    [OpenApiResponseWithBody(statusCode: HttpStatusCode.OK, contentType: "application/json", bodyType: typeof(IList<BookModel>))]
    [Function("HTTPListBooks")]
    public async Task<HttpResponseData> ListBooks([HttpTrigger(AuthorizationLevel.Function, "get", Route = "book")] HttpRequestData request)
    {
        _logger.LogInformation($"[{request.FunctionContext.InvocationId}] Processing request for list books endpoint.");
        ...
        if(!queryResult.Any())
        {
            _logger.LogWarning("The table 'books' is empty!");

            var errorResponse = request.CreateResponse(HttpStatusCode.InternalServerError);
            await errorResponse.WriteAsJsonAsync<ErrorModel>(new ErrorModel(
                Error: "No books found :(",
                ErrorMessage: @"There were no books in the table ""books"" "
            ));
            return errorResponse;
        }
        
        _logger.LogInformation("Table 'books' has at least one entry :)");
    ...
    [OpenApiOperation(operationId: "getBook", tags: new[] {"books"}, Summary = "Get Book", Description = "Get a book by its ISBN.")]
    [OpenApiSecurity("function_key", SecuritySchemeType.ApiKey, Name = "code", In = OpenApiSecurityLocationType.Query)]
    [OpenApiParameter(name: "isbn", In = ParameterLocation.Path, Required = true, Type = typeof(string), Summary = "ISBN of the reqeuested book")]
    [OpenApiResponseWithBody(statusCode: HttpStatusCode.OK, contentType: "application/json", bodyType: typeof(BookModel))]
    ...
    public async Task<HttpResponseData> GetBook([HttpTrigger(AuthorizationLevel.Function, "get", Route = "book/{isbn}")] HttpRequestData request, string isbn)
    {
        _logger.LogInformation($"[{request.FunctionContext.InvocationId}] Processing request to get specific book by its isbn with the isbn {isbn}.");
    ...
    // create new book
    [OpenApiOperation(operationId: "createBook", tags: new[] {"books"}, Summary = "Create Book", Description = "Create a new book in the backend.")]
    [OpenApiSecurity("function_key", SecuritySchemeType.ApiKey, Name = "code", In = OpenApiSecurityLocationType.Query)]
    ...
    public async Task<HttpResponseData> CreateBook([HttpTrigger(AuthorizationLevel.Function, "post", Route = "book")] HttpRequestData request)
    {
        _logger.LogInformation($"[{request.FunctionContext.InvocationId}] Processing request for create book endpoint.");
        ...
        if (_table.GetEntityIfExists<BookTableModel>(partitionKey: string.Empty, rowKey: createBookReq.Isbn).HasValue)
        {
            var errorResponse = request.CreateResponse(HttpStatusCode.BadRequest);
            await errorResponse.WriteAsJsonAsync<ErrorModel>(new
            (
                Error: "Book Already Exists",
                ErrorMessage: "Can't create a book as one with that ISBN already exists!"
            ));
            return errorResponse;
        }

        // transform BookModel into BookTableModel and write row to table; partition + row key need to be unique!
        var createTableRow = await _table.AddEntityAsync<BookTableModel>(new()
        {
    ...
    [OpenApiOperation(operationId: "deleteBook", tags: new[] {"books"}, Summary = "Delete Book", Description = "Delete a book by its ISBN.")]
    [OpenApiSecurity("function_key", SecuritySchemeType.ApiKey, Name = "code", In = OpenApiSecurityLocationType.Query)]
    [OpenApiParameter(name: "isbn", In = ParameterLocation.Path, Required = true, Type = typeof(string), Summary = "ISBN of the to be deleted book")]
    ...
    public async Task<HttpResponseData> DeleteBook([HttpTrigger(AuthorizationLevel.Function, "delete", Route = "book/{isbn}")] HttpRequestData request, string isbn)
    {
        _logger.LogInformation($"[{request.FunctionContext.InvocationId}] Processing request to delete specific book by its isbn with the isbn {isbn}.");
